...
router.get('/GetCheckPoints/:CheckListID/:MouldID', (req, res) => {

    const { CheckListID, MouldID } = req.params;
    ...
    const query = `
        SELECT
            ...
            p.LastUpdatedTime,
            c.CheckListName
        FROM Mould_Execute_PMCheckPoint p
        INNER JOIN Config_Mould_PMCheckList c
            ON p.CheckListID = c.CheckListID
        WHERE p.CheckListID = @CheckListID
          AND p.MouldID = @MouldID
          AND p.CheckListType = 1
    `;

    const sqlRequest = new sql.Request();

    sqlRequest.input('CheckListID', sql.Int, CheckListID);
    sqlRequest.input('MouldID', sql.NVarChar(100), MouldID);

    sqlRequest.query(query, (err, result) => {
        if (err) {
            return middlewares.standardResponse(res, null, 500, err.message);
        }

        return middlewares.standardResponse(res, result.recordset, 200, "Success");
    });
});
...
router.post('/UpdateCheckPointStatus', async (req, res) => {
  ...
  try {
    ...
    sqlRequest.input('MouldID', sql.NVarChar(100), resolvedMouldID);
